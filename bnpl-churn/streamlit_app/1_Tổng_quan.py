# -*- coding: utf-8 -*-
"""Trang 1 — Tổng quan khách hàng BNPL (EDA, dùng bảng đặc trưng toàn kỳ).
Chạy: streamlit run streamlit_app/1_Tổng_quan.py"""
...
# Trang này là EDA nên dùng bnpl_customer_features.csv (43 cột, toàn kỳ 2024).
# bnpl_model_features.csv (đặc trưng tại T_ref) chỉ dùng cho Trang 3 — xem mục 3.2.2 báo cáo.
...
REQUIRED_COLS = ["lifecycle_segment", "churn_label", "top_category", "city_tier",
                 "recency_days", "promo_rate"]
_missing = [c for c in REQUIRED_COLS if c not in feat.columns]
if _missing:
    st.error("Bảng đặc trưng toàn kỳ thiếu cột: " + ", ".join(_missing)
             + ". Kiểm tra load_customer_features() trong app_utils.py đang đọc đúng "
               "data/bnpl_customer_features.csv (không phải bnpl_model_features.csv).")
    st.caption("Các cột hiện có: " + ", ".join(map(str, feat.columns)))
    st.stop()
