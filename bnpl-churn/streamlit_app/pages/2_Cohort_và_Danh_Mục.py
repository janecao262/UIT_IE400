...
st.caption(
    "Cohort tính theo tháng có giao dịch đầu tiên, chỉ gồm khách kích hoạt từ 01/2024 "
...
st.caption(
    f"Trung bình trên {len(mature)} cohort đã quan sát được ít nhất {MATURE_MIN_PERIODS} tháng vòng đời "
...
section(
    "Retention và churn theo danh mục thanh toán",
...
if churn_by_cat is not None:
    ...
    with col1:
        ...
        colors = [
            ACCENT_2 if v > cbc["churn_rate"].mean() else ACCENT
            for v in cbc["churn_rate"]
        ]
    ...
    with col2:
        st.dataframe(
            cbc.rename(columns={"churn_rate": "Churn (%)", "n": "Số khách"}),
            width="stretch",
        )
...
if retention_curve_cat is not None:
    ...
    for cat in rc.index:
        ax.plot(
            rc.columns.astype(str), rc.loc[cat], marker="o", ms=3, lw=1.5, label=cat
        )
    ax.set_title("Đường cong retention trung bình theo danh mục")
    ax.set_xlabel("Tháng thứ n")
...
section(
    "Phân khúc vòng đời khách hàng (FPU / RPU)",
