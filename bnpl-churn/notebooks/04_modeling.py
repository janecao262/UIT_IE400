...
tx = pd.read_csv(
    f"{DATA_DIR}/bnpl_transactions_clean.csv", parse_dates=["transaction_date"]
)
cust = pd.read_csv(
    f"{DATA_DIR}/bnpl_customers_clean.csv", parse_dates=["activation_date"]
)
...
print(
    f"Cửa sổ nhãn: ({T_ref:%d/%m/%Y}, {OBS_DATE:%d/%m/%Y}] - {CHURN_WINDOW_DAYS} ngày"
)
...
print(
    f"Churn rate trên population này: {y_map.mean():.2%} ({y_map.sum():,}/{len(y_map):,})"
)
print(
...
def build_features(hist: pd.DataFrame, t_ref: pd.Timestamp) -> pd.DataFrame:
    ...
    for w in (30, 90, 180):
        f[f"freq_{w}d"] = (
            hist[hist.transaction_date > t_ref - pd.Timedelta(days=w)]
            .groupby("customer_id")
            .size()
        )
    f["monetary_90d"] = (
        hist[hist.transaction_date > t_ref - pd.Timedelta(days=90)]
        .groupby("customer_id")["amount_vnd"]
        .sum()
    )
    ...
    f["mean_inter_days"] = g["transaction_date"].apply(
        lambda s: s.diff().dt.days.mean()
    )
...
feat["account_age_days"] = (
    T_ref - cust.set_index("customer_id").loc[feat.index, "activation_date"]
).dt.days
...
data_dict = {
    ...
    "late_rate": "Tỷ lệ giao dịch trả trễ hạn",
    "partial_rate": "Tỷ lệ giao dịch trả một phần",
...
print(
    f"Train: {X_tr.shape} | Test: {X_te.shape} | churn train: {y_tr.mean():.2%} | scale_pos_weight = {spw:.2f}"
)
        ...
        [
            {
                ...
                "CV AUC (mean)": (
                    s := cross_val_score(
                        m, X_tr, y_tr, cv=cv, scoring="roc_auc", n_jobs=-1
                    )
                ).mean(),
                "CV AUC (std)": s.std(),
            }
...
print(
    classification_report(
        y_te, best_model.predict(X_te), target_names=["Không churn (0)", "Churn (1)"]
    )
)
...
plt.tight_layout()
plt.savefig(f"{FIG_DIR}/bnpl_confusion_matrix.png", bbox_inches="tight")
plt.show()

def clean_names(pipe):
    return [
        n.split("__", 1)[1] for n in pipe.named_steps["pre"].get_feature_names_out()
    ]

imp = {
    name: pd.Series(
        fitted[name].named_steps["clf"].feature_importances_,
        index=clean_names(fitted[name]),
    )
...
plt.tight_layout()
plt.savefig(f"{FIG_DIR}/bnpl_feature_importance.png", bbox_inches="tight")
plt.show()

lr_coefs = pd.Series(
    fitted["Logistic Regression"].named_steps["clf"].coef_[0],
    index=clean_names(fitted["Logistic Regression"]),
).sort_values()
...
ax.set_xlabel("Ngưỡng phân lớp")
ax.set_ylabel("Giá trị")
ax.set_title(f"Precision & Recall theo ngưỡng - {best_name}")
ax.legend()
plt.tight_layout()
plt.savefig(f"{FIG_DIR}/bnpl_threshold_analysis.png", bbox_inches="tight")
plt.show()

num_stats = {
    c: {
        "min": float(np.nanmin(X[c])),
        "med": float(np.nanmedian(X[c])),
        "max": float(np.nanmax(X[c])),
    }
    for c in num_cols
}
...
with open(f"{MODELS_DIR}/metrics_bnpl.json", "w", encoding="utf-8") as f:
    json.dump(
        {
            k: artifact[k]
            for k in ["best_name", "metrics_test", "churn_rate", "trained_at"]
        },
        f,
        ensure_ascii=False,
        indent=2,
